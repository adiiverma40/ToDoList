...
function createTask() {
  ...
  newTask.classList.add("outsideList");
  orderList.appendChild(newTask);
...
function toggleButton(newTask){
  ...
  function completedTask(newTask) {
    ...
    try{compTaskBtn.addEventListener("click", () => {
    ...
    localStorage.setItem("task", JSON.stringify(task));
      ...
      console.log(err);
      console.log("error is because their are not any button in the screen dont worry");
    }
  }

  function deleteTask(taskElement){
    let deleteBtn = document.querySelector(".dlt-btn");
   try{ deleteBtn.addEventListener("click", () => {
    taskElement.remove();
    let task = JSON.parse(localStorage.getItem("task"));
    ...
    localStorage.setItem("task", JSON.stringify(task));
      ...
      console.log(err);
      console.log("error is because their are not any button in the screen don't worry");
    }
